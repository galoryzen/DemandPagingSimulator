...
def start_iterations(tam_marco, so, proc, marcos, marcos_so, instrucciones):
    ...
    tabla = ttk.Label(content, text=print_tablapag(
        tabla_pagina), font=('Helvetica', 14))
    ...
    labelsPaginas = [ttk.Label(
        content, text=f'{pagina*tam_marco}-{(pagina*tam_marco + (tam_marco-1))}') for pagina in paginas_proc]
    labelsMP = [ttk.Label(
        content, text=f'{marco*tam_marco}-{(marco*tam_marco + (tam_marco-1))}') for marco in memoria]
    labelsDirLogica = [ttk.Label(content, text='         ')
                       for marco in memoria]

    bs = [Button(content, text=f'Página {pagina}', width=10,
                 pady=pady, bg='white') for pagina in paginas_proc]
    ms = [Button(content, text=f'Marco {marco}', width=10, pady=pady, bg='yellow') if marco in marcos_so else Button(
        content, text=f'Marco {marco}', width=10, pady=pady, bg='white') for marco in memoria]

    incrementarVIbt = Button(content, text=f'+ Velocidad Proceso', width=20,
                             pady=pady, bg='#082032', fg="#DDDDDD", command=incrementarVI)
    incrementarANbt = Button(content, text=f'+ Velocidad Animaciones',
                             width=20, pady=pady, bg='#082032', fg="#DDDDDD", command=incrementarAN)
    disminuirVIbt = Button(content, text=f'- Velocidad Proceso',
                           width=20, pady=pady, bg='#FF4C29', command=disminuirVI)
    disminuirANbt = Button(content, text=f'- Velocidad Animaciones',
                           width=20, pady=pady, bg='#FF4C29', command=disminuirAN)
    ...
    for instruccion in instrucciones:
        ...
        labelinstruccion.config(
            text=f'Instrucción:      Dirección {instruccion[0]} {tipo}   >>>   Pagina {pag}')
        ...
        try:
            ...
            bitacora.config(
                text='El programa trató acceder a una dirección que no correspondía dentro de las de el')
        ...
        if info[1] == 0:
            ...
            if len(marcos) != 0:
                ...
                bitacora.config(
                    text=f'Ingresando la página {pag} al marco {m}')
                salida_archivo += f'Ingresando la página {pag} al marco {m}\n'
                content.after(speed)
                root.update()
                content.update()

                buttons = [widget for widget in content.winfo_children(
                ...
                bo_copy = Button(
                    content, text=bo['text'], width=10, pady=pady, bg='blue')
                ...
                bitacora.config(
                    text=f'Cambiando el bit de valido/invalido a 1')
                ...
                bitacora.config(
                    text=f'La página menos usada recientemente es la página {LRU}, liberando asi el marco {m}')
                ...
                if tabla_pagina[LRU][2] == 1:
                    bitacora.config(
                        text='El bit sucio es 1, por tanto hay swap out')
                    ...
                    bitacora.config(
                        text='El bit sucio es 0, por tanto NO hay swap out')
                ...
                #SWAP IN
                bitacora.config(
                    text=f'Swap in de la página {pag} al marco {m}')
                ...
                bitacora.config(
                    text=f'Borrando la fila de la página {LRU} en la tabla de página')
            ...
            if instruccion[1] == 'E':
                ...
                bitacora.config(
                    text=f'Cambiando el bit sucio de la página {pag} a 1')
            ...
            bitacora.config(
                text=f'La página se encontraba ya cargada, especificamente en el marco {tabla_pagina[pag][0]}')
    ...
    labelinstruccion.config(
        text=f'No. Fallos de página: {fallos},   No. reemplazos: {reemplazos}')
...
def tomar_datos():
    ...
    Label(root, text="Marcos disponibles separados por espacio",
          font=('Helvetica', 14)).pack()

    marcos.pack(pady=10)
    marcos.insert(
        0, 'EJ: 7 9 10, que no entren en conflicto con los marcos del SO')
    # marcos.insert(0, a4)
    marcos.configure(state='disabled')

    ttk.Frame(content, width=800, height=50).pack()
    Label(root, text="Seleccione su archivo de instrucciones: ",
          font=('Helvetica', 14)).pack()

    select_file_button = Button(root, text="Seleccionar", command=select_file,
                                pady=2, bg='darkorange', font=('Helvetica', 14)).pack()
    ttk.Frame(content, width=800, height=50).pack()
    ruta = Label(root, text=file_name, font=('Helvetica', 12))
    ruta.pack()

    ttk.Frame(content, width=800, height=40).pack()
    my_button2 = Button(root, text="Ingresar", command=check_values, font=(
        "Helvetica", 24), fg="#DDDDDD", bg="darkolivegreen").pack()

    warn = Label(root, text="-", font=("Helvetica", 12), pady=30).pack()

    tam_marco_focus_in = tam_marco.bind(
        '<Button-1>', lambda x: on_focus_in(tam_marco))
...
def check_values():
    ...
    try:
        ...
        wlabel.config(
            text="Debe ingresar un número en el campo del tamaño del marco")
        return

    if tam_marco <= 0:
        wlabel.config(
            text="Debe ingresar un número positivo en el campo del tamaño del marco")
        tam_marco = None
        return
    ...
    try:
        ...
        wlabel.config(
            text="Debe ingresar un número en el campo del tamaño del SO")
        return

    if so <= 0:
        wlabel.config(
            text="Debe ingresar un número positivo en el campo del tamaño del SO")
        return
    ...
    try:
        ...
        wlabel.config(
            text="Debe ingresar un número en el campo del tamaño del proceso")
        return

    if proc <= 0:
        wlabel.config(
            text="Debe ingresar un número positivo en el campo del tamaño del proceso")
        return
    ...
    try:
        ...
        wlabel.config(
            text="Debe ingresar números enteros diferentes separados por espacio, EJ: 7 9 10")
        return
    ...
    if a & b:
        wlabel.config(
            text=f'Los marcos libres están en conflictos con los marcos que utilizará el SO que son: {" ".join(temp)}\n')
        return
    ...
    try:
        ...
        wlabel.config(
            text=f'Hubo un problema con su archivo, por favor carguelo de nuevo')
        return
...
my_button = Button(content, text="Iniciar", command=tomar_datos, font=(
    "Helvetica", 24), fg="#DDDDDD", bg="#FF4C29").pack()
